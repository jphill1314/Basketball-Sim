...
class Foul (homeTeamHasBall: Boolean, timeRemaining: Int, shotClock: Int, homeTeam: Team, awayTeam: Team, playerWithBall: Int, location: Int, var foulType: FoulType):
    ...
    private fun getFloorFoul(): Int{
        ...
        if(foulType == FoulType.OFF_BALL){
            ...
            if(r.nextBoolean()){
                ...
                val aggression = (offense.getPlayerAtPosition(playerWithBall).aggressiveness + ((offense.aggression + 10) * 5) / 2.0)
                ...
                val aggression = (defense.getPlayerAtPosition(playerWithBall).aggressiveness + ((defense.aggression + 10) * 5) / 2.0)
    ...
    private fun getShootingFoul(): Int{
        ...
        val defenderAbility: Double = when(foulType){
            FoulType.SHOOTING_CLOSE -> {(defender.onBallDefense + defender.postDefense) / 2.0}
            FoulType.SHOOTING_MID -> {(defender.onBallDefense + defender.postDefense + defender.perimeterDefense) / 3.0}
            FoulType.SHOOTING_LONG -> {(defender.onBallDefense + defender.perimeterDefense) / 2.0}
            else -> 0.0
        }
